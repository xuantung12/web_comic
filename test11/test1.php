...
if (isset($_POST['add_manga'])) {
    ...
    // Kiểm tra xem truyện đã tồn tại chưa dựa trên tiêu đề
    ...
    if ($result_check->num_rows > 0) {
        ...
        // Nếu truyện chưa tồn tại, tiếp tục thêm truyện mới
        ...
        if ($_FILES['cover_image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($_FILES['cover_image']['tmp_name'], $target)) {
                ...
                $sql = "INSERT INTO manga (title, description, cover_image, author, statuss, is_featured, comic_for_boy, comic_for_girl) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                ...
                if ($stmt->execute()) {
                    ...
                    // Lưu title và cover_image vào session
                    $_SESSION['title'] = $title;
                    $_SESSION['cover_image'] = $cover_image;
                    $_SESSION['statuss'] = $statuss;
                    $_SESSION['description']= $description;
                    $_SECTION['author'] =  $author;
                    
                    // Chuyển hướng để ngăn việc gửi lại biểu mẫu
                    header('Location: test1.php?status=success');
...
//////////////
if (!empty($genre)) {
    // Lưu genre vào session
    $_SESSION['selected_genre'] = $genre;
}
...
if ($_SESSION['avatar']) {
    echo "<img src='" . htmlspecialchars($_SESSION['avatar']) . "' alt='Avatar' width='100'>";
}
echo "<p>Đây là trang test1.php</p>";
        ...
        <div class="slides-wrapper">
            <?php while ($row = $featured_result->fetch_assoc()): ?>
                <div class="featured-item">
                    <a href="view_chapter.php?manga_id=<?php echo $row['id']; ?>" class="silde-item">
        ...
        <?php if ($result->num_rows > 0): ?>
            <?php while ($row = $result->fetch_assoc()): ?>
                ...
                <div class="action-buttons">
                    <a href="test1.php?delete=<?php echo $row['id']; ?>" class="delete-button" onclick="return confirm('Bạn có chắc chắn muốn xóa truyện này?')">Xóa</a>
